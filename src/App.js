...
function App() {
  ...
  const today = displayWeatherData?.forecast?.forecastday?.[0]?.day;
  const highC = today?.maxtemp_c ?? Math.round(displayWeatherData?.current?.temp_c + 3);
  const lowC = today?.mintemp_c ?? Math.round(displayWeatherData?.current?.temp_c - 5);
            ...
            <div className="flex items-end space-x-4">
              <div className="flex items-baseline">
                <span className="text-6xl font-thin leading-none">
                  {Math.round(displayWeatherData.current.temp_c)}
                </span>
                <span className="text-2xl font-thin opacity-75">°C</span>
              </div>
              <div className="pb-2">
                ...
                <div className="text-xs opacity-60 space-y-0.5">
                  <div>H: {Math.round(highC)}°C L: {Math.round(lowC)}°C</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className={`absolute bottom-20 md:bottom-6 right-6 z-20 ${textColor}`} style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
            <div className="flex flex-col space-y-3 text-right text-sm">
              ...
              <div className="flex items-center justify-end space-x-2">
                ...
                <span className="font-light">{Math.round(displayWeatherData.current.wind_kph)} km/h</span>
              </div>
              <div className="flex items-center justify-end space-x-2">
                <span className="opacity-60">FEELS</span>
                <span className="font-light">{Math.round(displayWeatherData.current.feelslike_c)}°C</span>
              </div>
              <div className="flex items-center justify-end space-x-2">
                <span className="opacity-60">VISIBILITY</span>
                <span className="font-light">{Math.round(displayWeatherData.current.vis_km)} km</span>
              </div>
            </div>
          </div>
        </>
